<?php 
	...
	if(isset($_GET['idekstrakurikuler'])){

		$ekstrakurikuler = mysqli_query($conn, "SELECT gambar FROM ekstrakurikuler WHERE id = '".$_GET['idekstrakurikuler']."' ");

		if(mysqli_num_rows($ekstrakurikuler) > 0){

			$p = mysqli_fetch_object($ekstrakurikuler);

			if(file_exists("../uploads/ekstrakurikuler/".$p->gambar)){

				unlink("../uploads/ekstrakurikuler/".$p->gambar);

			}

		}

		$delete = mysqli_query($conn, "DELETE FROM ekstrakurikuler WHERE id = '".$_GET['idekstrakurikuler']."' ");
